...
const DropdownFilter: React.FC<DropdownFilterProps> = ({ label, items, selectedItems, setSelectedItems }) => {
  ...
  const filteredItems = items.filter((item) =>
    item.toLowerCase().includes(search.toLowerCase()) && !selectedItems.includes(item)
  );
  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" className="flex items-center gap-2 border-[#27272A] min-w-[150px] h-10">
          {label}
          <ChevronDown />
        </Button>
      </DropdownMenuTrigger>
        ...
        <div className="relative">
          <div ref={chipContainerRef} className="absolute left-3 top-2 flex flex-wrap gap-1 max-h-24 overflow-y-auto">
            {selectedItems.map((item) => (
              <span key={item} className="bg-[#262626] text-white text-xs px-2 py-1 rounded-md flex items-center">
                {item}
          ...
          <Input
            ...
            style={{ height: `${inputHeight}px`, paddingTop: selectedItems.length > 0 ? "40px" : "10px" }}
          />
        </div>
